...
auth.onAuthStateChanged(async (user) => {
  if (user) {
    ...
    try {
      ...
      initTimeClock();
      // Show Operations V2 button for all roles
      ['btn-ops-v2', 'fab-ops-v2-btn'].forEach(id => {
...
async function startCameraStream() {
  ...
  // On iPhones with multiple rear cameras (11+), 'exact' facingMode can accidentally select
  // the telephoto lens, making the preview appear permanently zoomed in.
  // Fix: prefer non-exact facingMode with a 4:3 resolution hint — iOS then defaults to the
  // main wide-angle camera. Fall back progressively. zoom is applied via applyConstraints
...
// ---- State ----
let _opsV2Open = false;
let _opsV2Tab = 'dashboard';
let _opsV2ClockInterval = null;
let _opsV2ChatUnsub = null;
let _opsV2ChecklistUnsub = null;
let _opsV2TasksCache = [];
let _opsV2TaskFilter = 'all';
let _opsV2DispatchFilter = 'all';
let _opsV2ChecklistDate = '';
let _opsV2Users = null;
let _opsV2ChatSeenCount = 0;
let _opsV2ChatTotalCount = 0;

// ---- Open / Close ----
function openOpsV2(tab) {
  _opsV2Open = true;
  const overlay = $('ops-v2-overlay');
  if (overlay) overlay.style.display = 'flex';
  _startOpsV2Clock();
  _applyOpsV2RoleVisibility();
  switchOpsV2Tab(tab || _opsV2Tab || 'dashboard');
}
window.openOpsV2 = openOpsV2;
...
function closeOpsV2() {
  ...
  if (overlay) overlay.style.display = 'none';
  _stopOpsV2Clock();
  if (_opsV2ChatUnsub) { _opsV2ChatUnsub(); _opsV2ChatUnsub = null; }
  if (_opsV2ChecklistUnsub) { _opsV2ChecklistUnsub(); _opsV2ChecklistUnsub = null; }
}
window.closeOpsV2 = closeOpsV2;

// ---- Clock ----
function _startOpsV2Clock() {
  _stopOpsV2Clock();
  const tick = () => {
    const el = $('ops-v2-clock');
    if (!el) return;
    const now = new Date();
    el.textContent = now.toLocaleTimeString('en-US', {
      timeZone: APP_TIMEZONE, hour: '2-digit', minute: '2-digit', second: '2-digit'
    }) + ' HST';
  };
  tick();
  _opsV2ClockInterval = setInterval(tick, 1000);
}
function _stopOpsV2Clock() {
  if (_opsV2ClockInterval) { clearInterval(_opsV2ClockInterval); _opsV2ClockInterval = null; }
}

// ---- Role Visibility ----
function _applyOpsV2RoleVisibility() {
  const isAdmin = currentUserRole === 'admin';
  const isManager = currentUserRole === 'manager';
  const canDispatch = isAdmin || isManager;
  // Hide dispatch tab for regular users
  document.querySelectorAll('.ops-v2-tab-dispatch').forEach(t => {
    t.style.display = canDispatch ? '' : 'none';
  });
  // If on dispatch tab and no access, redirect
  if (_opsV2Tab === 'dispatch' && !canDispatch) switchOpsV2Tab('tasks');
}

// ---- Tab Switching ----
function switchOpsV2Tab(tab) {
  _opsV2Tab = tab;
  // Update tab buttons
  document.querySelectorAll('#ops-v2-tab-bar .ops-v2-tab').forEach(btn => {
    btn.classList.toggle('active', btn.dataset.tab === tab);
  });
  // Hide all panels
  ['dashboard','dispatch','tasks','checklist','chat'].forEach(t => {
    const p = $('ops-v2-panel-' + t);
    if (p) p.style.display = 'none';
  });
  const panel = $('ops-v2-panel-' + tab);
  if (panel) panel.style.display = tab === 'chat' ? 'flex' : 'block';

  // Load data for tab
  if (tab === 'dashboard') loadOpsV2Dashboard();
  else if (tab === 'dispatch') loadOpsV2Dispatch();
  else if (tab === 'tasks') loadOpsV2Tasks();
  else if (tab === 'checklist') loadOpsV2Checklist();
  else if (tab === 'chat') loadOpsV2Chat();
}
window.switchOpsV2Tab = switchOpsV2Tab;

// ================================================================
// DASHBOARD TAB
// ================================================================
async function loadOpsV2Dashboard() {
  _renderOpsV2KPIs();
  _renderOpsV2PriorityList();
}

function _renderOpsV2KPIs() {
  try {
    const available = vehiclesCache.filter(v => v.tripStatus === 'available').length;
    const onTrip = vehiclesCache.filter(v => v.tripStatus === 'on-trip' || v.tripStatus === 'private-trip' || v.tripStatus === 'scheduled').length;
    const shop = vehiclesCache.filter(v => v.tripStatus === 'repair-shop').length;
    const total = vehiclesCache.length;
    _setKPI('kpi-available', available);
    _setKPI('kpi-ontrip', onTrip);
    _setKPI('kpi-shop', shop);
    _setKPI('kpi-total', total);
  } catch(e) {
    ['kpi-available','kpi-ontrip','kpi-shop','kpi-total'].forEach(id => _setKPI(id,'�'));
  }
  // Task KPIs from loaded cache
  _updateOpsV2TaskKPIs();
}
...
function _updateOpsV2TaskKPIs() {
  const today = todayDateString();
  const urgent = _opsV2TasksCache.filter(t => !t.done && (t.priority === 'urgent' || t.priority === 'critical')).length;
  const overdue = _opsV2TasksCache.filter(t => !t.done && t.dueDate && t.dueDate < today).length;
  const mine = _opsV2TasksCache.filter(t => !t.done && t.assignedTo === (currentUser && currentUser.uid)).length;
  _setKPI('kpi-urgent', urgent);
  _setKPI('kpi-overdue', overdue);
  _setKPI('kpi-mine', mine);
  // Update tasks tab badge
  const urgentBadge = $('ops-v2-tasks-badge');
  if (urgentBadge) {
    if (urgent > 0) { urgentBadge.textContent = urgent; urgentBadge.style.display = ''; }
    else urgentBadge.style.display = 'none';
  }
}

async function _renderOpsV2PriorityList() {
  const container = $('ops-v2-priority-list');
  if (!container) return;
  container.innerHTML = '<p class="ops-v2-empty">Loading�</p>';
  try {
    if (_opsV2TasksCache.length === 0) await _loadOpsV2TasksFromDB();
    const today = todayDateString();
    const priority = _opsV2TasksCache
      .filter(t => !t.done && (
    ...
    if (!priority.length) {
      container.innerHTML = '<p class="ops-v2-empty">? No urgent or overdue tasks!</p>';
      return;
    }
    container.innerHTML = priority.map(t => {
      const priClass = t.priority === 'urgent' || t.priority === 'critical' ? 'pri-urgent'
        : t.priority === 'high' ? 'pri-high' : 'pri-normal';
      const overdue = t.dueDate && t.dueDate < today ? ' � ? Overdue' : (t.dueDate === today ? ' � ?? Due today' : '');
      const assignee = t.assigneeName ? ` � ?? ${escapeHtml(t.assigneeName)}` : '';
      return `<div class="ops-v2-priority-item ${priClass}" onclick="switchOpsV2Tab('tasks')">
        <div class="ops-v2-priority-item-body">
          <div class="ops-v2-priority-title">${escapeHtml(t.title)}</div>
          <div class="ops-v2-priority-meta">${escapeHtml(t.priority||'normal')}${overdue}${assignee}</div>
        </div>
      </div>`;
    }).join('');
  } catch(e) {
    container.innerHTML = '<p class="ops-v2-empty">Error loading tasks</p>';
  }
}
...
// DISPATCH TAB
// ================================================================
async function loadOpsV2Dispatch() {
  const grid = $('ops-v2-dispatch-grid');
  if (!grid) return;
  grid.innerHTML = '<p class="ops-v2-empty" style="padding:24px;">Loading fleet�</p>';
  try {
    // Refresh from DB
    const snap = await db.collection('vehicles').orderBy('plate').get();
    const fresh = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    // Update vehiclesCache so other parts stay in sync
    fresh.forEach(fv => {
      const idx = vehiclesCache.findIndex(v => v.id === fv.id);
      if (idx >= 0) Object.assign(vehiclesCache[idx], fv);
    });
    _renderDispatchGrid(fresh);
  } catch(e) {
    grid.innerHTML = '<p class="ops-v2-empty">Error loading fleet</p>';
  }
}
window.loadOpsV2Dispatch = loadOpsV2Dispatch;

function _renderDispatchGrid(vehicles) {
  const grid = $('ops-v2-dispatch-grid');
  if (!grid) return;
  let list = vehicles;
  if (_opsV2DispatchFilter !== 'all') {
    list = list.filter(v => {
      const s = v.tripStatus || 'available';
      if (_opsV2DispatchFilter === 'available') return s === 'available';
      if (_opsV2DispatchFilter === 'on-trip') return s === 'on-trip' || s === 'private-trip' || s === 'scheduled';
      if (_opsV2DispatchFilter === 'repair-shop') return s === 'repair-shop';
      return true;
    });
  }
  if (!list.length) { grid.innerHTML = '<p class="ops-v2-empty">No vehicles match this filter.</p>'; return; }
  const canEdit = currentUserRole === 'admin' || currentUserRole === 'manager';
  grid.innerHTML = list.map(v => _vehicleCardHtml(v, canEdit)).join('');
}

function _vehicleCardHtml(v, canEdit) {
  const s = v.tripStatus || 'available';
  const statusLabel = { available:'? Available', 'on-trip':'?? On Trip', scheduled:'?? Scheduled', 'private-trip':'?? Private Trip', 'repair-shop':'?? In Shop', parked:'??? Parked' }[s] || s;
  const statusClass = s === 'available' ? 'vsb-available' : s === 'on-trip' || s === 'private-trip' ? 'vsb-on-trip' : s === 'scheduled' ? 'vsb-scheduled' : s === 'repair-shop' ? 'vsb-repair' : 'vsb-other';
  const cardClass = s === 'available' ? 'vc-available' : (s === 'on-trip' || s === 'private-trip') ? 'vc-on-trip' : s === 'repair-shop' ? 'vc-repair' : 'vc-other';
  const driverHtml = v.currentDriver ? `<div class="ops-v2-vehicle-driver">?? ${escapeHtml(v.currentDriver)}</div>` : '';
  const editHtml = canEdit ? `<div class="ops-v2-vehicle-actions">
    <select class="ops-v2-status-select" onchange="changeVehicleStatusV2('${v.id}',this.value)">
      <option value="available" ${s==='available'?'selected':''}>? Available</option>
      <option value="on-trip" ${s==='on-trip'?'selected':''}>?? On Trip</option>
      <option value="scheduled" ${s==='scheduled'?'selected':''}>?? Scheduled</option>
      <option value="repair-shop" ${s==='repair-shop'?'selected':''}>?? In Shop</option>
      <option value="parked" ${s==='parked'?'selected':''}>??? Parked</option>
    </select>
  </div>` : '';
  return `<div class="ops-v2-vehicle-card ${cardClass}">
    <div class="ops-v2-vehicle-plate">${escapeHtml(v.plate||'�')}</div>
    ${v.name ? `<div class="ops-v2-vehicle-name">${escapeHtml(v.name)}</div>` : ''}
    <span class="ops-v2-vehicle-status-badge ${statusClass}">${statusLabel}</span>
    ${driverHtml}
    ${editHtml}
  </div>`;
}

async function changeVehicleStatusV2(vid, newStatus) {
  if (currentUserRole !== 'admin' && currentUserRole !== 'manager') return;
  try {
    await db.collection('vehicles').doc(vid).update({ tripStatus: newStatus });
    const idx = vehiclesCache.findIndex(v => v.id === vid);
    if (idx >= 0) vehiclesCache[idx].tripStatus = newStatus;
    toast('Vehicle status updated', 'success');
    // Re-render
    _renderDispatchGrid(vehiclesCache);
  } catch(e) {
    toast('Failed to update vehicle status', 'error');
  }
}
window.changeVehicleStatusV2 = changeVehicleStatusV2;

function filterDispatch(filter, btn) {
  _opsV2DispatchFilter = filter;
  document.querySelectorAll('#ops-v2-panel-dispatch .ops-v2-filter-btn').forEach(b => b.classList.remove('active'));
  if (btn) btn.classList.add('active');
  _renderDispatchGrid(vehiclesCache);
}
window.filterDispatch = filterDispatch;

// ================================================================
// TASKS TAB
// ================================================================
async function _loadOpsV2TasksFromDB() {
  const snap = await db.collection('followups')
    .where('status', '!=', 'done')
    .orderBy('status')
    .orderBy('createdAt', 'desc')
    .get();
  _opsV2TasksCache = snap.docs.map(d => ({ id: d.id, ...d.data() }));
  _updateOpsV2TaskKPIs();
}

async function loadOpsV2Tasks() {
  const container = $('ops-v2-tasks-list');
  if (!container) return;
  container.innerHTML = '<p class="ops-v2-empty">Loading tasks�</p>';
  try {
    await _loadOpsV2TasksFromDB();
    await _loadOpsV2Users();
    _renderOpsV2Tasks();
    _updateOpsV2TaskKPIs();
  } catch(e) {
    container.innerHTML = '<p class="ops-v2-empty">Error loading tasks</p>';
    console.error('OpsV2 tasks error:', e);
  }
}

async function _loadOpsV2Users() {
  if (_opsV2Users) return;
  try {
    const snap = await db.collection('users').orderBy('displayName').get();
    _opsV2Users = [];
    snap.forEach(doc => {
      const d = doc.data();
      _opsV2Users.push({ id: doc.id, name: d.displayName || d.email || doc.id });
    });
    // Populate assignee select
    const sel = $('ops-v2-task-assignee');
    if (sel) {
      sel.innerHTML = '<option value="">Team (all)</option>' +
        _opsV2Users.map(u => `<option value="${u.id}">${escapeHtml(u.name)}</option>`).join('');
    }
  } catch(e) { _opsV2Users = []; }
}

function _renderOpsV2Tasks() {
  const container = $('ops-v2-tasks-list');
  if (!container) return;
  const today = todayDateString();
  const uid = currentUser && currentUser.uid;
  let tasks = [..._opsV2TasksCache];
  if (_opsV2TaskFilter === 'mine') tasks = tasks.filter(t => t.assignedTo === uid || (!t.assignedTo));
  else if (_opsV2TaskFilter === 'urgent') tasks = tasks.filter(t => t.priority === 'urgent' || t.priority === 'critical');
  else if (_opsV2TaskFilter === 'overdue') tasks = tasks.filter(t => t.dueDate && t.dueDate < today);
  // Sort: urgent first, then by due date
  tasks.sort((a,b) => {
    if (a.done !== b.done) return a.done ? 1 : -1;
    const rank = { critical:0, urgent:1, high:2, normal:3, low:4 };
    const rd = (rank[a.priority]||3) - (rank[b.priority]||3);
    if (rd !== 0) return rd;
    if (a.dueDate && b.dueDate) return a.dueDate.localeCompare(b.dueDate);
  ...
  if (!tasks.length) { container.innerHTML = '<p class="ops-v2-empty">No tasks in this view. ?</p>'; return; }
  container.innerHTML = tasks.map(t => _taskCardHtml(t, today)).join('');
}

function _taskCardHtml(t, today) {
  const done = t.done || t.status === 'done';
  const p = t.priority || 'normal';
  const tagClass = p === 'urgent' || p === 'critical' ? 'tag-urgent' : p === 'high' ? 'tag-high' : p === 'low' ? 'tag-low' : 'tag-normal';
  const tcClass = p === 'urgent' || p === 'critical' ? 'tc-urgent' : p === 'high' ? 'tc-high' : p === 'low' ? 'tc-low' : 'tc-normal';
  const isOverdue = t.dueDate && t.dueDate < today && !done;
  const dueHtml = t.dueDate ? `<span class="ops-v2-task-due${isOverdue?' overdue':''}">${isOverdue?'? Overdue: ':'?? '}${t.dueDate}</span>` : '';
  const assigneeHtml = t.assigneeName ? `<span class="ops-v2-task-assignee">?? ${escapeHtml(t.assigneeName)}</span>` : '';
  return `<div class="ops-v2-task-card ${tcClass}${done?' done':''}">
    <div class="ops-v2-task-checkbox${done?' checked':''}" onclick="toggleOpsV2Task('${t.id}','${done?'undo':'done'}')" title="${done?'Mark active':'Mark done'}">${done?'?':''}</div>
    <div class="ops-v2-task-body">
      <div class="ops-v2-task-title${done?' done-text':''}">${escapeHtml(t.title)}</div>
      <div class="ops-v2-task-meta">
        <span class="ops-v2-task-tag ${tagClass}">${escapeHtml(p)}</span>
        ${dueHtml}${assigneeHtml}
      </div>
    </div>
  </div>`;
}

async function toggleOpsV2Task(taskId, action) {
  try {
    const isDone = action === 'done';
    await db.collection('followups').doc(taskId).update({
      done: isDone,
      status: isDone ? 'done' : 'pending',
      doneAt: isDone ? firebase.firestore.FieldValue.serverTimestamp() : null,
      doneBy: isDone ? (currentUser && currentUser.uid) : null
    });
    const t = _opsV2TasksCache.find(x => x.id === taskId);
    if (t) { t.done = isDone; t.status = isDone ? 'done' : 'pending'; }
    _renderOpsV2Tasks();
    _updateOpsV2TaskKPIs();
    toast(isDone ? 'Task marked done ?' : 'Task re-opened', 'success');
  } catch(e) { toast('Failed to update task', 'error'); }
}
window.toggleOpsV2Task = toggleOpsV2Task;

function filterOpsV2Tasks(filter, btn) {
  _opsV2TaskFilter = filter;
  document.querySelectorAll('#ops-v2-panel-tasks .ops-v2-filter-btn').forEach(b => b.classList.remove('active'));
  if (btn) btn.classList.add('active');
  _renderOpsV2Tasks();
}
window.filterOpsV2Tasks = filterOpsV2Tasks;

function openOpsV2AddTask() {
  const form = $('ops-v2-add-task-form');
  if (form) { form.style.display = ''; $('ops-v2-task-title') && $('ops-v2-task-title').focus(); }
}
window.openOpsV2AddTask = openOpsV2AddTask;

function closeOpsV2AddTask() {
  const form = $('ops-v2-add-task-form');
  if (form) form.style.display = 'none';
}
window.closeOpsV2AddTask = closeOpsV2AddTask;

async function submitOpsV2Task() {
  const title = ($('ops-v2-task-title')||{}).value && $('ops-v2-task-title').value.trim();
  if (!title) { toast('Task title is required', 'warning'); return; }
  const priority = ($('ops-v2-task-priority')||{}).value || 'normal';
  const dueDate = ($('ops-v2-task-due')||{}).value || null;
  const assignSel = $('ops-v2-task-assignee');
  const assignedTo = assignSel ? assignSel.value : '';
  const assigneeName = assignedTo ? ((_opsV2Users||[]).find(u=>u.id===assignedTo)||{}).name || '' : '';
  try {
    const btn = document.querySelector('#ops-v2-add-task-form .ops-v2-btn-primary');
    if (btn) { btn.disabled = true; btn.textContent = 'Saving�'; }
    const doc = await db.collection('followups').add({
      title, priority, dueDate, assignedTo: assignedTo||null, assigneeName: assigneeName||null,
      status: 'pending', done: false,
      createdAt: firebase.firestore.FieldValue.serverTimestamp(),
      createdBy: currentUser && currentUser.uid,
      createdByName: (currentUser && currentUser.displayName) || ''
    });
    _opsV2TasksCache.unshift({ id: doc.id, title, priority, dueDate, assignedTo:assignedTo||null, assigneeName:assigneeName||null, status:'pending', done:false });
    $('ops-v2-task-title').value = '';
    if ($('ops-v2-task-due')) $('ops-v2-task-due').value = '';
    closeOpsV2AddTask();
    _renderOpsV2Tasks();
    _updateOpsV2TaskKPIs();
    toast('Task created ?', 'success');
  } catch(e) { toast('Failed to save task', 'error'); console.error(e); }
  finally {
    const btn = document.querySelector('#ops-v2-add-task-form .ops-v2-btn-primary');
    if (btn) { btn.disabled = false; btn.textContent = 'Save Task'; }
  }
}
window.submitOpsV2Task = submitOpsV2Task;

// ================================================================
// CHECKLIST TAB
// ================================================================
const OPS_V2_CHECKLIST_DEFAULTS = [
  { section: 'Morning', items: ['Check all vehicles for damage / cleanliness', 'Verify all vehicles returned from previous day', 'Review urgent tasks & assign team priorities'] },
  { section: 'Vehicles', items: ['Confirm all scheduled trips are dispatched', 'Check fuel levels for vehicles in use', 'Flag any vehicles needing maintenance'] },
  { section: 'Team', items: ['All team members checked in / accounted for', 'Review any open incidents or follow-ups', 'End-of-shift handoff notes completed'] }
];

async function loadOpsV2Checklist() {
  const today = todayDateString();
  _opsV2ChecklistDate = today;
  const dateEl = $('ops-v2-checklist-date');
  if (dateEl) dateEl.textContent = "Today's Checklist � " + new Date(today+'T12:00:00').toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric'});

  // Unsubscribe previous listener
  if (_opsV2ChecklistUnsub) { _opsV2ChecklistUnsub(); _opsV2ChecklistUnsub = null; }

  const docRef = db.collection('opsV2Checklists').doc(today);
  const snap = await docRef.get();
  if (!snap.exists) {
    // Create default checklist for today
    const items = [];
    OPS_V2_CHECKLIST_DEFAULTS.forEach(sec => {
      sec.items.forEach(text => {
        items.push({ id: _opsV2GenId(), section: sec.section, text, done: false, doneBy: null, doneByName: null });
      });
    });
    await docRef.set({ date: today, items, createdAt: firebase.firestore.FieldValue.serverTimestamp() });
  }
  // Real-time listener
  _opsV2ChecklistUnsub = docRef.onSnapshot(snap => {
    const data = snap.data() || {};
    _renderOpsV2Checklist(data.items || []);
  }, () => {});
}

function _renderOpsV2Checklist(items) {
  const container = $('ops-v2-checklist-items');
  if (!container) return;
  const done = items.filter(i=>i.done).length;
  const progressEl = $('ops-v2-checklist-progress');
  if (progressEl) {
    progressEl.textContent = `${done} / ${items.length} completed`;
    _setKPI('kpi-checklist', items.length ? `${done}/${items.length}` : '�');
  }
  if (!items.length) { container.innerHTML = '<p class="ops-v2-empty">No checklist items</p>'; return; }
  let html = '';
  let lastSection = null;
  items.forEach(item => {
    if (item.section && item.section !== lastSection) {
      html += `<div class="ops-v2-check-section">${escapeHtml(item.section)}</div>`;
      lastSection = item.section;
    }
    const doneByHtml = item.done && item.doneByName ? `<span class="ops-v2-check-done-by">? ${escapeHtml(item.doneByName)}</span>` : '';
    const canDelete = currentUserRole === 'admin' || currentUserRole === 'manager';
    const deleteBtn = canDelete && !item.section ? `<button class="ops-v2-check-delete" onclick="deleteOpsV2CheckItem('${item.id}')" title="Delete">?</button>` : '';
    html += `<div class="ops-v2-check-item${item.done?' checked':''}">
      <input type="checkbox" ${item.done?'checked':''} onchange="toggleOpsV2CheckItem('${item.id}',this.checked)">
      <span class="ops-v2-check-text">${escapeHtml(item.text)}</span>
      ${doneByHtml}${deleteBtn}
    </div>`;
  });
  container.innerHTML = html;
}

async function toggleOpsV2CheckItem(itemId, checked) {
  try {
    const docRef = db.collection('opsV2Checklists').doc(_opsV2ChecklistDate);
    const snap = await docRef.get();
    const data = snap.data() || {};
    const items = (data.items || []).map(i => i.id === itemId ? {
      ...i, done: checked,
      doneBy: checked ? (currentUser&&currentUser.uid) : null,
      doneByName: checked ? ((currentUser&&currentUser.displayName)||'') : null
    } : i);
    await docRef.update({ items });
  } catch(e) { toast('Failed to update checklist', 'error'); }
}
window.toggleOpsV2CheckItem = toggleOpsV2CheckItem;

function openOpsV2AddCheckItem() {
  const form = $('ops-v2-add-check-form');
  if (form) { form.style.display=''; $('ops-v2-check-text')&&$('ops-v2-check-text').focus(); }
}
window.openOpsV2AddCheckItem = openOpsV2AddCheckItem;

function closeOpsV2AddCheckItem() {
  const form = $('ops-v2-add-check-form');
  if (form) form.style.display='none';
}
window.closeOpsV2AddCheckItem = closeOpsV2AddCheckItem;

async function submitOpsV2CheckItem() {
  const text = ($('ops-v2-check-text')||{}).value && $('ops-v2-check-text').value.trim();
  if (!text) { toast('Enter an item', 'warning'); return; }
  try {
    const docRef = db.collection('opsV2Checklists').doc(_opsV2ChecklistDate);
    const snap = await docRef.get();
    const data = snap.data() || {};
    const items = [...(data.items||[]), { id: _opsV2GenId(), section: null, text, done: false, doneBy: null, doneByName: null }];
    await docRef.update({ items });
    $('ops-v2-check-text').value = '';
    closeOpsV2AddCheckItem();
    toast('Item added', 'success');
  } catch(e) { toast('Failed to add item', 'error'); }
}
window.submitOpsV2CheckItem = submitOpsV2CheckItem;

async function deleteOpsV2CheckItem(itemId) {
  if (currentUserRole !== 'admin' && currentUserRole !== 'manager') return;
  try {
    const docRef = db.collection('opsV2Checklists').doc(_opsV2ChecklistDate);
    const snap = await docRef.get();
...
window.deleteOpsV2CheckItem = deleteOpsV2CheckItem;

function _opsV2GenId() {
...
// CHAT TAB
// ================================================================
async function loadOpsV2Chat() {
  if (_opsV2ChatUnsub) return; // already listening
  const container = $('ops-v2-chat-messages');
  if (!container) return;
  container.innerHTML = '<p class="ops-v2-empty" style="text-align:center;padding:30px;">Loading�</p>';

  _opsV2ChatUnsub = db.collection('opsV2Chat')
    .orderBy('sentAt', 'asc')
    .limitToLast(100)
    .onSnapshot(snap => {
      _opsV2ChatTotalCount = snap.size;
      _renderOpsV2Chat(snap.docs);
    }, err => {
      const c = $('ops-v2-chat-messages');
      if (c) c.innerHTML = '<p class="ops-v2-empty">Chat unavailable</p>';
    });
}

function _renderOpsV2Chat(docs) {
  const container = $('ops-v2-chat-messages');
  if (!container) return;
  if (!docs.length) { container.innerHTML = '<p class="ops-v2-empty" style="text-align:center;padding:40px;">No messages yet. Say hello! ??</p>'; return; }
  const myUid = currentUser && currentUser.uid;
  let html = '';
  let lastDate = null;
  docs.forEach(doc => {
    const d = doc.data();
    const ts = d.sentAt ? (d.sentAt.toDate ? d.sentAt.toDate() : new Date(d.sentAt)) : new Date();
    const dateStr = ts.toLocaleDateString('en-US', {month:'short',day:'numeric',year:'numeric'});
    if (dateStr !== lastDate) {
      html += `<div class="ops-v2-chat-date-sep">${dateStr}</div>`;
      lastDate = dateStr;
    }
    const isMe = d.uid === myUid;
    const initials = (d.senderName||'?').split(' ').map(n=>n[0]||'').join('').toUpperCase().slice(0,2);
    const timeStr = ts.toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit'});
    html += `<div class="ops-v2-chat-msg${isMe?' mine':''}">
      <div class="ops-v2-chat-avatar" title="${escapeHtml(d.senderName||'')}">${initials}</div>
      <div class="ops-v2-chat-bubble-wrap">
        ${!isMe ? `<span class="ops-v2-chat-sender">${escapeHtml(d.senderName||'Team')}</span>` : ''}
        <div class="ops-v2-chat-bubble">${escapeHtml(d.text||'')}</div>
        <span class="ops-v2-chat-time">${timeStr}</span>
      </div>
    </div>`;
  });
  container.innerHTML = html;
...
async function sendOpsV2Chat() {
  const input = $('ops-v2-chat-input');
  if (!input) return;
  const text = input.value.trim();
  if (!text) return;
  ...
  input.value = '';
  try {
    await db.collection('opsV2Chat').add({
      text,
      uid: currentUser.uid,
      senderName: currentUser.displayName || currentUser.email || 'Team Member',
      sentAt: firebase.firestore.FieldValue.serverTimestamp()
    });
  } catch(e) {
...
window.sendOpsV2Chat = sendOpsV2Chat;
