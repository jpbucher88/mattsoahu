...
function renderLocationsWidget() {
  ...
  // Get all home locations, preserving known order
  ...
  // â”€â”€ Combined per-location sections â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  for (const loc of allLocations) {
    ...
    // â€” Needs Cleaning sub-section â€”
  ...
  if (onTrip.length > 0) {
    ...
    for (const v of onTrip) {
      ...
      if (v.tripReturnDate) {
        ...
        returnLabel = `<span class="trip-return-label${isOverdue ? ' trip-overdue' : ''}">â†© ${rd.toLocaleString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit', hour12: true, timeZone: APP_TIMEZONE })}${isOverdue ? ' OVERDUE' : ''}</span>`;
        if (isOverdue) {
    ...
    try {
      for (const item of failItems) {
        ...
        await db.collection('incidents').add({
          ...
          type: 'damage',
          title: 'File Claim: ' + item.label + (notes ? ' â€” ' + notes : ''),
...
$('btn-save-location').addEventListener('click', async () => {
  ...
  if (((wasOnTrip && nowHome) || nowReturnFromRepair) && !selectedVehicle.photoExcluded) {
    ...
    // Kapiolani vehicles need parking paid on return
    if (homeLocation === '1585 Kapiolani') {
      updateData.needsParking = true;
    }
...
window.vehicleReturned = async function(vehicleId) {
  ...
  try {
    ...
    if (v.homeLocation === '1585 Kapiolani') parkUpdate.needsParking = true;
    await db.collection('vehicles').doc(vehicleId).update(parkUpdate);
    // Update local cache
    Object.assign(v, {
      tripStatus: 'home',
      needsCleaning: true,
...
function renderTaskAgenda(allItems) {
  ...
  if (currentTaskTab === 'compliance') {
    ...
    if (compGroupEl) {
      ...
      if (items.length === 0) {
        compGroupEl.innerHTML = '<div class="agenda-empty"><p class="hint">No compliance tasks. âœ…</p></div>';
      } else {
        ...
        for (const group of groups) {
          ...
          html += `<div class="compliance-group-card${isOver ? ' compliance-group-overdue' : ''}">
            <div class="compliance-group-header">
              <span class="compliance-group-plate">ðŸš— ${escapeHtml(plate)}</span>
  ...
  overlay.innerHTML = `
    <div class="note-edit-modal">
      ...
      <div class="task-log-section">
        <div class="note-edit-section-label" style="margin-bottom:6px;">ðŸ“‹ Activity Log</div>
  ...
  try {
    ...
    resultsEl.innerHTML = `
      <div class="prod-summary-row">
        ...
        <strong>${fmtDate(rangeStart)} â€” ${fmtDate(rangeEnd)}</strong>
  ...
  try {
    ...
    list.innerHTML = docs.map(doc => {
      ...
      const followUpHTML = d.followUpDate
        ? `<div class="inc-followup">ðŸ“… Follow-up: <strong>${d.followUpDate}</strong></div>`
        : '';
...
window.deleteExpense = async function(docId) {
  if (currentUserRole !== 'admin') {
    // managers can only delete their own â€” server-side check on load
